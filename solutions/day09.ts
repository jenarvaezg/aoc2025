...
export function part2(input: string): number {
  ...
  for (let i = 0; i < red.length; i++) {
    const [x1, y1] = red[i]!;
    const [x2, y2] = red[(i + 1) % red.length]!;
    polygonEdges.push({ x1, y1, x2, y2, vertical: x1 === x2 });
  }
  ...
  const pointInPolygon = ([px, py]: Point) => {
    let inside = false;
    for (let i = 0; i < red.length; i++) {
      const [x1, y1] = red[i]!;
      const [x2, y2] = red[(i + 1) % red.length]!;
  ...
  const rectangleInside = (p1: Point, p2: Point) => {
    ...
    if (!corners.every(pointInPolygon)) return false;
